...
ts_phase$campaign <- 
  factor(ts_phase$campaign, levels = c("MJ22", "MA23", "MA24", "SO24"))

# csv
write.csv(ts_phase, "G:/My Drive/Research/Projects/DC_auto/Data/Result/GPP_SR_cospectral/raw_64d_one_GPP/df_lag_hr/ts_phase.csv", row.names = F)
...
par_raw_phase$campaign <- 
  factor(par_raw_phase$campaign, levels = c("MJ22", "MA23", "MA24", "SO24"))

write.csv(par_raw_phase, "G:/My Drive/Research/Projects/DC_auto/Data/Result/GPP_SR_cospectral/raw_64d_one_GPP/df_lag_hr/PAR_raw_phase.csv", row.names = F)


gpp_raw_phase <- purrr::map_dfr(gpp_raw_files, phase_diff_csv)

gpp_raw_phase$campaign <- 
  factor(gpp_raw_phase$campaign, levels = c("MJ22", "MA23", "MA24", "SO24"))

write.csv(gpp_raw_phase, "G:/My Drive/Research/Projects/DC_auto/Data/Result/GPP_SR_cospectral/raw_64d_one_GPP/df_lag_hr/GPP_raw_phase.csv", row.names = F)
...
phase_diff_graph <- function(df, res, driver){
  ...
  df %>% 
  filter(resp == res) %>% 
  ggplot(aes(campaign, Phase_hr_avg)) +
  geom_point(size = 5) + 
  geom_errorbar(
    aes(campaign,
